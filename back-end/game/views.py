...
def main_lobby(request):
    exist_user = user.objects.get(id = id)
    return render(request, 'game/main_lobby.html', {"exist_user":exist_user})

def game_lobby(request):
    card = {
        'attributes': attributes
    }
    return render(request, 'game/game_lobby.html', card)
